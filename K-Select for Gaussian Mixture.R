# CS 274A | Homework 6
# Automatically Selecting K for Gaussian Mixtures
# by Aydin Mohseni
...
# Create am empty vector in which to save the BIC score
# for each run of the algorithm
BIC <- c()

# Run the algorithm for K underlying components 
for (K in 1:5) {
  ...
  repeat {
    if (converged == TRUE) {
      ...
      if (t == 0) {
        ...
        if ( t > 1 ) { 
          if ( LLHt[t] - LLHt[t-1] < .01 ) { converged <- TRUE }
        }
  ...
  # Count the number of paramaters in the model (means, covariances, marginals)
  paramCount <- nrow(kMean)*ncol(kMean) + (K/2)*(nrow(covMatrix)^2) + K
  BIC[K] <- LLHt[t] - paramCount/2*log(N)
}

# Plot the BIC scores of the models as a function of the number of components K = 1,..,5
BICgraph <- data.frame(matrix(c(1:5, BIC), nrow = 5, ncol = 2))
colnames(BICgraph) <- c("K", "BIC")
g <- ggplot(BICgraph, aes(x = K, y = BIC)) + 
  geom_line() + ggtitle("BIC as a Function of Components K") +
  labs(y = "BIC", x="Components K")
print(g)